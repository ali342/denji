...
# MODIFIED: Removed the synchronous sqlite3 connect function
# def connect_db():
...
# MODIFIED: Converted to async and uses asyncpg
async def get_topup_record(user_id) :
    user_id = int(user_id)
    pool = await db.get_db_pool()
    async with pool.acquire() as conn:
        result = await conn.fetch('SELECT * FROM topup_requests WHERE user_id = $1', user_id)
...
async def get_account(update: Update, context: ContextTypes.DEFAULT_TYPE) :
    ...
    try:
        ...
        # MODIFIED: Replaced sqlite3 connection with asyncpg pool
        pool = await db.get_db_pool()
        async with pool.acquire() as conn:
            result = await conn.fetchrow('SELECT CAST(ROUND(balance, 2) AS DECIMAL(10,2)), name , username , CAST(ROUND(total_spent, 2) AS DECIMAL(10,2))   FROM users WHERE id = $1', int(id))
...
async def account(user_id) :
    pool = await db.get_db_pool()
    async with pool.acquire() as conn:
        result = await conn.fetchrow('SELECT name, CAST(ROUND(balance, 2) AS DECIMAL(10,2)), CAST(ROUND(total_spent, 2) AS DECIMAL(10,2))  FROM users WHERE id = $1', user_id)
...
# MODIFIED: Converted to async, takes asyncpg 'conn' instead of 'cursor'
async def cal(user , conn) :
    accepted_topup = 0
    successful_number_buy = 0
    successful_social_buy = 0
    added_removed_manual = 0
    
    data = await conn.fetchrow('SELECT name , username , balance FROM users WHERE id = $1',(user,))
    name, username ,balance = data

    top = await conn.fetch('''SELECT amount_added FROM topup_requests WHERE user_id = $1 AND (status = 'مقبول' OR status = 'approved' )''' , (user,))
    for op in top :
        amount = op[0]
        accepted_topup += amount
        
    numbers = await conn.fetch('''SELECT request_id, price FROM item_requests WHERE (status = 'منتهية' OR status = 'مفعل' OR code IS NOT NULL) AND user_id = $1 ''',(user,))
    for op in numbers :
        req , price = op
        successful_number_buy += price
        
    socials = await conn.fetch('''SELECT price , refunded FROM social_requests WHERE (status = 'مقبول' OR status = 'في الانتظار' OR status = 'مقبول جزئيا' )  AND user_id = $1 ''',(user,))
    for op in socials :
        price , refunded = op
        if refunded :
    ...
    manual = await conn.fetch('SELECT added_amount , type FROM added_manually WHERE user_id = $1 ',(user,))
    for op in manual :
        amount , type = op
        if type == 'إضافة رصيد' :
    ...
    total_revenue = accepted_topup + added_removed_manual - balance - successful_number_buy - successful_social_buy
...
async def record(update: Update, context: ContextTypes.DEFAULT_TYPE) :
    ...
    pool = await db.get_db_pool()
    async with pool.acquire() as conn:
        # NOTE: Using multiple placeholders as per the original logic's structure.
        users = await conn.fetch('SELECT id FROM users WHERE id <> $1 AND id <> $2 AND id <> $3 AND id <> $4 AND id <> $5', 5349543151, 5363898935, 6313379271, 1693441250, 6306691535)
        ...
        for user_record in users :
            user = user_record[0]
            # MODIFIED: Must now await the async 'cal' function
            name , username , balance , accepted_topup , added_removed_manual , successful_number_buy , successful_social_buy , total_revenue = await cal(user , conn)
